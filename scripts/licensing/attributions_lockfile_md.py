...
PI_NODE = ROOT / "adapters" / "pi-cli-bin" / "npm"
MARKDOWN_CODE_FENCE = "```\n"
...
def _node_lockfile_paths() -> tuple[Path, ...]:
    """Return every npm lockfile whose packages ship or build with the project."""
    return (_node_lockfile_path(), PI_NODE / "package-lock.json")
...
def _node_license_inventory() -> list[LicenseInventoryEntry]:
    """Return minimal Node.js dependency license rows from all npm lockfiles."""
...
def cmd_node() -> int:
    """Generate ATTRIBUTIONS-Node.md from every maintained npm lockfile."""
    out = ROOT / "ATTRIBUTIONS-Node.md"
    data, _ = _node_license_checker_data()
    packages: dict[str, NodeAttributionPackage] = {}
    for lockfile in _node_lockfile_paths():
        lock = cast(dict[str, Any], json.loads(lockfile.read_text(encoding="utf-8")))
        workspace_package_keys = _node_workspace_package_keys(lock)
