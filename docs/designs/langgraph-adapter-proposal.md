...
| `tools.enabled`       | The parent document's `None`/`[]`/group/member semantics apply, but only over context-supplied tools: there is no declared inventory to select from, so a `compiled` graph can only be all-or-nothing.                                                                                                                                          |
